...
public class CodeWriter implements AutoCloseable {
	...
	public void writeLine(String text) {
		try {
			...
			System.out.println(("failed to write line: " + text));
			e.printStackTrace();
		}
	}
	...
	public void writePushPop(Command command, String segment, Integer index) {
		...
		if (command.equals(Command.C_PUSH)) {
	...
	public void writeLabel(String label) {
		/*
		 * label label - This command labels the current location in the function’s
		 * code. Only labeled locations can be jumped to from other parts of the
		 * program. The scope of the label is the function in which it is defined. The
		 * label is an arbitrary string composed of any sequence of letters, digits,
	...
	public void writeIf(String label) {
		/*
		 * if-goto label - This command effects a conditional goto operation. The
		 * stack’s topmost value is popped; if the value is not zero, execution
		 * continues from the location marked by the label; otherwise, execution
		 * continues from the next command in the program. The jump destination must be
		 * located in the same function.
		 */
	...
	public void writeCall(String functionName, Integer numArgs) {
		/*
		 * push return-address - push LCL - push ARG - push THIS - push THAT - ARG =
		 * SP-n-5 - LCL = SP - goto f - (return-address) -
		 */
	...
	public void writeReturn() {
		/*
		 * FRAME = LCL RET = *(FRAME-5) ARG = pop() SP = ARG+1 THAT = *(FRAME-1) THIS =
		 * *(FRAME-2) ARG = *(FRAME-3) LCL = *(FRAME-4) goto RET
		 */
		writeLine("// return");
